...
#region ============ 1.5 ВЫБОР РЕЖИМА (для запуска как приложение) ============

# Когда .exe/скрипт запускают двойным кликом, режим явно не задан. Чтобы не
# менять систему «с порога», предлагаем интерактивное меню: сначала проверка.
# В неинтерактивной среде (CI) и при явном -DryRun меню не показывается.
if (-not $DryRun -and [Environment]::UserInteractive) {
    Write-Host ""
    Write-Host "Выберите режим работы:" -ForegroundColor Cyan
    Write-Host "  [1] Проверка — ничего не менять, только показать (рекомендуется)" -ForegroundColor White
    Write-Host "  [2] Оптимизировать — применить изменения" -ForegroundColor White
    $choice = Read-Host "Ваш выбор (1/2), Enter = 1"
    if ($choice -ne '2') {
        $DryRun = $true
        Write-Log "Выбран режим проверки (-DryRun)." 'DRY'
    } else {
        Write-Log "Выбрано применение изменений." 'INFO'
    }
}
